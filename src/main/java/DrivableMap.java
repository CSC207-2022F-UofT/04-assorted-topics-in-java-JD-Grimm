...
class DrivableMap {
     ...
     */
    public boolean addDrivable(String id, Drivable dr){
        if (drivable_map.containsKey(id)){
            return false;
        }
        drivable_map.put(id, dr);
        return true;
    }
    ...
    public boolean hasFasterThan(int speed){
        for(Drivable dr: this.drivable_map.values()){
            if (dr.getMaxSpeed() >= speed)
                return true;
        }
        return false;
    }
